namespace TCLoudExplorer.@class
{
    ...
    /// </summary>
    public record SizeFile : SizeItem
    {
    ...
    /// </summary>
    public record SizeDirectory : SizeItem
    {
        ...
        public static SizeDirectory FromPath(string path)
        {
            if (!Directory.Exists(path))
                throw new DirectoryNotFoundException($"Directory '{path}' does not exist");

            var rootPath = Path.GetFullPath(path);
            var rootItem = new SizeDirectory(rootPath, rootPath);
            return rootItem;
        }

        protected SizeDirectory(string fullPath, string rootPath) :
          base(fullPath, rootPath)
        {
            TotalFileCount = 0;
            TotalDirectoryCount = 0;
            ...
            try
            {
                ...
                foreach (var name in Directory.GetDirectories(fullPath))
                {
                    ...
                    // Not cancelled? Then recurse here:
                    var newDir = new SizeDirectory(name, rootPath);
                    Directories.Add(newDir);
                    // Add it to our size and counters
                    SizeInBytes += newDir.SizeInBytes;
